...
fpdtype_t line_sol[${order+1}][${nvars}], line_flux[${order+2}][${ndims}][${nvars}], 
          line_flux_LO[${order+2}][${ndims}][${nvars}], line_flux_HO[${order+2}][${ndims}][${nvars}], line_tflux[${order+2}][${ndims}][${nvars}];
fpdtype_t ul[${nvars}], ur[${nvars}], usd[${nvars}], n[${ndims}], t[${ndims}], tmp, p, v[${ndims}], alpha[${order+2}], salpha[${order+2}];
...
% for j in range(order+1):
    ...
    % for i in range(order):
        ...
        % for var in range(nvars):
            line_flux_LO[${i+1}][0][${var}] = n[0]*fntemp[${var}] + n[1]*fntemp2[${var}];
            line_flux_LO[${i+1}][1][${var}] = n[1]*fntemp[${var}] - n[0]*fntemp2[${var}];
        % endfor
        ...
        if (abs(res[${i+j*(order+1)}]) > ${e_max} 
            ...
            // Set HO flux to LO flux
            % for dim, var in pyfr.ndrange(ndims, nvars):
                line_flux_HO[${i+1}][${dim}][${var}] = line_flux_LO[${i+1}][${dim}][${var}];
            % endfor
            }
        else {
            alpha[${i+1}] = 0.0;
            ...
            % for dim, var in pyfr.ndrange(ndims, nvars):
                line_flux_HO[${i+1}][${dim}][${var}] = ftemp[${dim}][${var}];
            % endfor
        }
    ...
    // Smooth alpha
    % for i in range(order):
        salpha[${i+1}] = ${' + '.join('{mx}*alpha[{m}]'.format(m=m, mx=mx)
                   for m, mx in enumerate(linesmoothmat[i+1]) if mx != 0)};
        salpha[${i+1}] = max(alpha[${i+1}], salpha[${i+1}]);
    % endfor

    // Right interface point
    alpha_fpts[${1*(order+1) + j}] = salpha[${order+1}];
    // Left interface point
    alpha_fpts[${3*(order+1) + j}] = salpha[${0}];


    % for i in range(order):
        % for dim, var in pyfr.ndrange(ndims, nvars):
            line_flux[${i+1}][${dim}][${var}] = salpha[${i+1}]*line_flux_LO[${i+1}][${dim}][${var}] + (1.0 - salpha[${i+1}])*line_flux_HO[${i+1}][${dim}][${var}];
        % endfor
    % endfor
...
% for i in range(order+1):
    ...
    % for j in range(order):
        ...
        ${pyfr.expand('centered_rsolve','ul','ur','t','fntemp2')};
        % for var in range(nvars):
            line_flux_LO[${j+1}][0][${var}] = n[0]*fntemp[${var}] + n[1]*fntemp2[${var}];
            line_flux_LO[${j+1}][1][${var}] = n[1]*fntemp[${var}] - n[0]*fntemp2[${var}];
        % endfor
        ...
        if (abs(res[${i+(j+1)*(order+1)}]) > ${e_max} 
            ...
            % for dim, var in pyfr.ndrange(ndims, nvars):
                line_flux_HO[${j+1}][${dim}][${var}] = line_flux_LO[${j+1}][0][${var}];
            % endfor
            }
        else {
            alpha[${j+1}] = 0.0;
            ...
            ${pyfr.expand('inviscid_flux', 'usd', 'ftemp', 'p', 'v')};
            % for dim, var in pyfr.ndrange(ndims, nvars):
                line_flux_HO[${j+1}][${dim}][${var}] = ftemp[${dim}][${var}];
            % endfor
        }
    % endfor
    ...
    // Smooth alpha
    % for j in range(order):
        salpha[${j+1}] = ${' + '.join('{mx}*alpha[{m}]'.format(m=m, mx=mx)
                   for m, mx in enumerate(linesmoothmat[j+1]) if mx != 0)};
        salpha[${j+1}] = max(alpha[${j+1}], salpha[${j+1}]);
    % endfor

    // Bottom interface point
    alpha_fpts[${0*(order+1) + i}] = salpha[${order+1}];
    // Top interface point
    alpha_fpts[${2*(order+1) + i}] = salpha[${0}];

    % for j in range(order):
        % for dim, var in pyfr.ndrange(ndims, nvars):
            line_flux[${j+1}][${dim}][${var}] = salpha[${j+1}]*line_flux_LO[${j+1}][${dim}][${var}] + (1.0 - salpha[${j+1}])*line_flux_HO[${j+1}][${dim}][${var}];
        % endfor
    % endfor
