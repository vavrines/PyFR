...
<%pyfr:macro name='rsolve' params='ul, ur, n, nf'>
    ...
    // Get Thorber modified velocity
    fpdtype_t ml = sqrt((${pyfr.dot('vl[{i}]', 'vl[{i}]', i=ndims)})*${c['gamma']}*pl/ul[0]);
    fpdtype_t mr = sqrt((${pyfr.dot('vr[{i}]', 'vr[{i}]', i=ndims)})*${c['gamma']}*pr/ur[0]);
    fpdtype_t z = min(1.,max(ml,mr));
% for i in range(ndims):
    vls[${i}] = 0.5*(vl[${i}] + vr[${i}]) - 0.5*z*(vr[${i}] - vl[${i}]);
    vrs[${i}] = 0.5*(vl[${i}] + vr[${i}]) + 0.5*z*(vr[${i}] - vl[${i}]);
% endfor

    // Get the normal velocity jump
    fpdtype_t dvs = ${pyfr.dot('n[{i}]', 'vrs[{i}] - vls[{i}]', i=ndims)};
...
% for i in range(ndims):
    va[${i}] = (vls[${i}]*sqrt(ul[0]) + vrs[${i}]*sqrt(ur[0]))*invsqrulpur;
% endfor
