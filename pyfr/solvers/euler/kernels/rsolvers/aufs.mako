...
// AUFS vector Splitting Approach
<%pyfr:macro name='rsolve' params='ul, ur, n, nf'>
    ...
    fpdtype_t us = 0.5*(nvl + nvr) + (cl - cr)/${c['gamma'] - 1};
    fpdtype_t cs = 0.5*(cl + cr) + 0.25*${c['gamma'] - 1}*(nvl - nvr);
    
    // Get wave speeds
    fpdtype_t s1 = 0.5*(nvl + nvr);
    fpdtype_t s2 = (s1 > 0.) ? min(min(0.,nvl-cl),us-cs) : max(max(0.,us+cs),nvr+cr);
