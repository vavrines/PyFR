...
% if lambda_type == 'witherden':
    s = sqrt(${gamma}*(pl + pr)/(ul[0] + ur[0]))
                + 0.5*fabs(vl[0] + vr[0]);
% elif lambda_type == 'davis':
    fpdtype_t al = sqrt(${gamma}*pl/ul[0]);
    fpdtype_t ar = sqrt(${gamma}*pr/ur[0]);
    s = max(fabs(vl[0]) + al,fabs(vr[0]) + ar);
% elif lambda_type == 'guermond':
    fpdtype_t rl = ul[0];
    fpdtype_t rr = ur[0];
    fpdtype_t vxl = vl[0];
    fpdtype_t vxr = vr[0];
    ${pyfr.expand('guermond_speed','rl','vxl','pl','rr','vxr','pr','s')};
% endif
...
<%pyfr:macro name='lambda_max' params='ul, ur, s'>
    // Compute the left and right fluxes + velocities and pressures
    fpdtype_t fl[${nvars}], fr[${nvars}];
    fpdtype_t vl[${ndims}], vr[${ndims}];
    fpdtype_t pl, pr;
