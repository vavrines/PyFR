...
// AUFST vector Splitting Approach
<%pyfr:macro name='rsolve' params='ul, ur, n, nf'>
    ...
    psl[0] = 0.;
    psr[0] = 0.;
    du[0] = ${c['gamma']}*(pl - pr)/(2.*cb);
% for i in range(ndims):
    psl[${i}+1] = n[${i}]*pl;
    psr[${i}+1] = n[${i}]*pr;
    du[${i}+1] = ${c['gamma']}*(pl*vl[${i}] - pr*vr[${i}])/(2.*cb);
% endfor
    psl[${nvars}-1] = pl*nvl; 
    psr[${nvars}-1] = pr*nvr;
    du[${nvars}-1] = (${c['gamma']}/(2.0*cb))*((cb*cb/(${c['gamma']}*${c['gamma'] - 1}))*(pl-pr) + 0.5*(pl*ql - pr*qr));
