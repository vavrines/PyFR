...
class NodalMeshAssembler:
    ...
    def get_connectivity(self):
        ...
        for k, v in self._pfacespents.items():
            pbc[f'periodic_{k}_l'] = bpart[v[0]] 
            pbc[f'periodic_{k}_r'] = bpart[v[1]] 

        for lpent in pbc:
            for pftype in pbc[lpent]:
                lfnodes = pbc[lpent][pftype]
                lfpts = np.array([[self._nodepts[n] for n in fn] for fn in lfnodes])
